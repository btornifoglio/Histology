...
filename = '2602-2-3-plm-9-8bit';
filename2 = '2602-2-3-plm-9'; %merged PLM to overlay later
S = 50;    % Size of the square subregion (pixels)
Thresh = 100000;
iImage = imread([filename '.tif'], 'tif');   % Import the image
iImage2 = imread([filename2 '.tif'], 'tif');
...
%colormap(gray);
hold on;
quiver(FiberPosX, -FiberPosY+D(1), FiberOrientationX, -FiberOrientationY, 0.5, 'y', 'LineWidth', .5);
hold off;
...
display([Thresh S]);
display([MA MVL CSD]);
saveas(gcf,'IVC2-C6-2-10X-PLM-MATFIBRE.tif')
% Save results
clear phi N1 N2 E C;
